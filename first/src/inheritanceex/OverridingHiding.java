...
class Car {
    ...
    static void printCompany(int num) {//1번 7번  static이므로 부모자식간 공유됨
        System.out.println(num +"번(Car의 printCompany()메소드): 현대 자동차");
    }
     
    public void printInfo(int num) {//5번
        System.out.printf("\n"+ num +"번(Car의 printInfo()메소드):  "
        + "차종:%s \t제조사:%s \t가격:%d \t", carName,company,price);
    }
    
    public void printInfo1(int num) {//6번
        System.out.printf("\n"+ num +"번(Car의 printInfo()메소드):  "
        + "차종:%s \t제조사:%s \t가격:%d \t", carName,company,price);
    }
}
...
class Truck extends Car {
    ...
    public static void printCompany(int num) {//2번 8번
    ...
    // 부모의 메소드를 오버라이딩
    public void printInfo(int num) {//9번
        super.printInfo(num);
        System.out.printf("(Truck의 printCompany()메소드) 적재중량:%d", maxLoad);
    }
    
    public void printInfo1(int num) {//10번
